...
- `ducting-scheme.md` — **start here for the duct design**: the scheme as it stands, why it is shaped that way, and the case against a second air handler. `ducting-register-schedule.md` is the editable per-register schedule. `ducting-design.md` is the as-built seed and `ducting-plan.md` the earlier forward spec.
