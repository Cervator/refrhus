...
**Supply airflow:** 588 CFM (at 50°F supply-air rise)
...
_Total conditioned volume **16,457 ft³** — the infiltration basis. Set `levels.<name>.height_ft` to correct a height Sweet Home 3D defaulted. A level contributing 0 ft³ added nothing to that total; usually that is deliberate (it holds no conditioned rooms — garage, crawlspace, joist space), but it is not the only cause: the volume is accumulated while walking each level's walls, so a level with rooms and no walls drawn is skipped and reads 0 ft³ here too._
...
| crawlspace | 0.50 → 27.5°F | `vented: false` (built-in default) | 0.50 → 8.0°F | `vented: false` (built-in default) — winter shorthand, reused for summer |
...
### Borrowed assembly U-values

| Category | Area | U used | Borrowed from | Why that is a stand-in |
|---|---:|---:|---|---|
| `buffer_floor` | 293.5 ft² | 0.05 | `assemblies.floor` | a slab's effective whole-area U can be an order of magnitude below a framed floor's |

_These surfaces are loaded at a number nobody measured for them. A borrow keeps the engine usable on a side-car written before the category existed, but it is a stand-in: declare `assemblies.<category>` to replace it with a real value._
...
- Not a separate problem from the one above: this area is inside the 293.5 ft² on the `buffer_floor` row of *Borrowed assembly U-values* (which spans every `buffer_floor` surface, drawn or not), so the two figures nest rather than add.
...
- Design load (heating): **31,757 BTU/hr = 2.6 tons**
- Recommended (smallest size that meets the load): **3.0 tons** (+13% vs load)
- Next size up: **3.5 tons** (+32% vs load)
- Existing unit: **4.0 tons** → +51% vs load → **oversized** ⚠
...
|---|---:|---:|---:|
| Utility Room | 5,222 | 551 | 97 |
| Main Bed | 3,963 | 2,032 | 94 |
| Kitchen | 4,857 | 1,796 | 90 |
| Future Media Room | 4,686 | 588 | 87 |
| Living room | 3,742 | 1,564 | 72 |
| Office | 2,199 | 1,003 | 46 |
| Play Room | 2,308 | 842 | 43 |
| Kids Room | 1,405 | 828 | 38 |
| Main Bath | 800 | 371 | 17 |
| Upper Bath | 544 | 199 | 10 |
| Main Closet | 454 | 109 | 8 |
| Bathroom | 448 | 27 | 8 |
| Upstairs Hallway | 402 | 141 | 7 |
| Closet | 222 | 77 | 4 |
| Closet | 197 | 62 | 4 |
| Closet | 173 | 53 | 3 |
| **16 rooms** | | | **630** |

_Each room's load is from the exterior walls, windows, doors and ceiling/floor attributed to it, plus infiltration on its own volume; design CFM is the larger of heating/cooling airflow. Served rooms sum to **630 CFM** vs the whole-house **588 CFM** — the gap is space not carried here: floor area not yet drawn as rooms (halls, stairs, unfinished), plus tiny rooms below the 3-CFM run threshold. Draw more rooms and it closes._
...
|---|---:|---:|---:|---:|---:|---:|
| main trunk | 630 | 12.2″ | **14″** | 589 fpm | — | — |
| Kitchen | 90 | 5.8″ | **6″** | 458 fpm | 45 ft | 0.036″ |
| Main Bed | 94 | 5.9″ | **6″** | 479 fpm | 39 ft | 0.032″ |
| Living room | 72 | 5.4″ | **6″** | 369 fpm | 26 ft | 0.021″ |
| Main Closet | 8 | 2.4″ | **4″** | 96 fpm | 30 ft | 0.024″ |
| Kids Room | 38 | 4.2″ | **5″** | 281 fpm | 42 ft | 0.034″ |
| Main Bath | 17 | 3.1″ | **4″** | 197 fpm | 32 ft | 0.025″ |
| Closet | 3 | 1.7″ | **4″** | 37 fpm | 38 ft | 0.030″ |
| Office | 46 | 4.5″ | **5″** | 341 fpm | 48 ft | 0.039″ |
| Upper Bath | 10 | 2.6″ | **4″** | 115 fpm | 39 ft | 0.031″ |
| Closet (2) | 4 | 1.8″ | **4″** | 47 fpm | 53 ft | 0.043″ |
| Play Room | 43 | 4.4″ | **5″** | 313 fpm | 41 ft | 0.033″ |
| Closet (3) | 4 | 1.7″ | **4″** | 42 fpm | 58 ft | 0.047″ |
| Upstairs Hallway | 7 | 2.3″ | **4″** | 85 fpm | 27 ft | 0.022″ |
| Utility Room | 97 | 6.0″ | **7″** | 362 fpm | 16 ft | 0.012″ |
| Future Media Room | 87 | 5.8″ | **6″** | 442 fpm | 15 ft | 0.012″ |
| Bathroom | 8 | 2.4″ | **4″** | 95 fpm | 15 ft | 0.012″ |
...
- **Some U-values are borrowed, not declared** — `buffer_floor` has no `assemblies` entry and stands in on a related assembly's number; the *Borrowed assembly U-values* table above names each donor and how far off it can be.
