...
- **The load has risen at almost every correction.** 31,757 → 37,962 → 42,097, then down to 39,694 only because the basement was finally *measured* rather than estimated. The sizing verdict on the existing 4-ton unit has flipped between oversized and well-matched three times.

**A sizing verdict that flips with each correction is one to hold loosely.** 3.5 tons is what today's model says; 3.8 is what one known-missing input alone would make it; and the remaining gaps have not been priced at all. Rounding to the next tier is the cheap direction to be wrong in — an oversized heat pump with inverter turndown short-cycles far less than a single-stage unit would, while an undersized one has no remedy short of replacement.
